...
Aside from the technical front, a significant part of our goal is increased accessibility. To do that, we must partner with local governments to gain access to sample ballots, so that we may construct segmentation masks for our analysis. As part of that, we plan to build a segmentation tool that will drastically reduce the man-hours it currently takes for us to draw information from the raw ballot. A mock-up of the segmentation UI is attached below.

![Segmentation UI](segmentationUI.png)
